...
4. After a successful HTTP-2xx batch response, `commands/update-custom-properties/index.js` walks `response.data.responses` and logs any sub-response that did not succeed, plus a `{ total, failures }` summary. Each sub-response is `{ success: true }` on success or `{ error: { status, code, message } }` on failure — the per-sub-request `status` lives **inside** `error`, not at the top level — so the failure test is `success !== true` and the full sub-response (carrying `error.{status,code,message}`) is logged. Per-sub-request errors do **not** trigger retry or backoff — the outer HTTP call succeeded.
